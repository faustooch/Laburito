...
function Header() {
  ...
  return (
    <header className="w-full border-b border-neutral-800/80 bg-neutral-950/80 sticky top-0 z-50 transition-all">
          ...
          <div className="hidden md:flex flex-grow justify-center px-8">
            <div className="relative w-full max-w-sm group">
              ...
              <input 
                ...
                className="w-full bg-neutral-900/40 border border-neutral-800 text-sm text-neutral-200 pl-10 pr-4 py-2 rounded-full focus:outline-none focus:border-slate-500/60 focus:bg-neutral-900/80 transition-all placeholder:text-neutral-600"
              />
            </div>
          </div>
        )}
        ...
        <nav className="flex items-center gap-3">
          {user ? (
            <div className="flex items-center gap-4">
              ...
              <Link to="/profile" className="relative p-0.5 rounded-full border border-neutral-800 hover:border-slate-500/60 transition-all duration-300 bg-neutral-900 group hover:-500/10">
            ...
            <div className="flex items-center gap-3 relative z-10">
              ...
              {location.pathname !== '/register' && (
                <Link to="/register" className="bg-slate-200 text-slate-950 text-sm font-bold px-4 py-1.5 sm:px-5 sm:py-2 rounded-full hover:bg-white transition-all hover:-200/20 active:scale-95">
                  Registrate
                </Link>
              )}
      ...
      {user && (
        <div className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out ${isMobileSearchOpen ? 'max-h-24 border-t border-neutral-800/80 opacity-100' : 'max-h-0 opacity-0'}`}>
          <div className="px-6 py-3 bg-neutral-950/95">
            <div className="relative w-full group">
              ...
              <input 
                ...
                onChange={(e) => setSearchTerm(e.target.value)}
                onKeyDown={handleSearch}
                className="w-full bg-neutral-900/60 border border-neutral-800 text-sm text-neutral-200 pl-10 pr-4 py-2.5 rounded-full focus:outline-none focus:border-slate-500/60 transition-all placeholder:text-neutral-600"
              />
            </div>
          </div>
        </div>
      ...
      {!user && isHomePage && (
        <div className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out ${isMobileNavOpen ? 'max-h-48 border-t border-neutral-800/80 opacity-100' : 'max-h-0 opacity-0'}`}>
          <nav className="flex flex-col px-6 py-4 gap-4 bg-neutral-950/95">
