  ...
  return (
    <header className="w-full border-b border-neutral-800/80 bg-neutral-950/80 backdrop-blur-xl sticky top-0 z-50 transition-all">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        {/* 3. Navegación central (NO logueados) */}
        {!user && isHomePage && (
        ...
        <nav className="flex items-center gap-3">
          {user ? (
            <div className="flex items-center gap-5">
              <button onClick={handleLogout} className="hidden sm:block text-xs font-bold uppercase tracking-widest text-neutral-500 hover:text-slate-200 transition-colors cursor-pointer">
                Salir
              </button>
            ...
            <div className="flex items-center gap-3 relative z-10">
              {location.pathname !== '/login' && (
                <Link to="/login" className="text-sm font-medium text-neutral-400 hover:text-slate-200 transition-colors px-3 py-2">
                  Ingresar
                </Link>
              )}
              {location.pathname !== '/register' && (
                <Link to="/register" className="bg-slate-200 text-slate-950 text-sm font-bold px-5 py-2 rounded-full hover:bg-white transition-all shadow-md hover:shadow-slate-200/20 active:scale-95">
                  Registrate
                </Link>
              )}
            </div>
