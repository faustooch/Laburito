...
def authenticate_google(db: Session, auth_code: str):
    ...
    # Si Google no devuelve id_token no se puede verificar firma/audience.
    if not token_info.get("id_token"):
        logger.error("Google token exchange sin id_token. keys=%s", list(token_info.keys()))
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Google no devolvió id_token en el intercambio del código."
        )
    ...
    try:
        ...
        logger.error("Fallo verify_oauth2_token: %s", str(e))
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
